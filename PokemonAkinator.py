...
def start_game(version):
    ...
    has_multiple_types = ask_question(multiple_type_question)
    number_question += 1
    ...
    while len(possible_generations) != 1: 
        ...
        if len(possible_generations) == 2: 
            if version == 0:
                ...
                in_generation = ask_question("GEN " + str(possible_generations[0]))
        else:
            if version == 0:
                in_generation = ask_question("Is the pokemon from generation " + str(possible_generations[0]) + 
                                     " to " + str(possible_generations[(len(possible_generations)//2) - 1]) + "?")
            else: 
                in_generation = ask_question("GEN_FROM " + str(possible_generations[0]) + 
                                     " to " + str(possible_generations[(len(possible_generations)//2) - 1]))
        number_question += 1
    ...
    if version == 0:
        ...
        greater_than_avergae = ask_question("BASE_STAT " + str(average_base_stat_total))
    opernad = "<="
        ...
        while not found_type: 
            ...
            if maxValue == most_common_type_value or (checked_resist and checked_weak):
                ...
                if version == 0:
                    ...
                    answer = ask_question("HAVE_TYPE " + most_common_type)

                num_asked += 1
                if answer:
                    found_type = True
                    KB = filter_KB("type:=:" + most_common_type, KB)
                    return KB, most_common_type, type_check_stack, resist_check_stack, weak_check_stack, num_asked
                else:
                    KB = filter_KB("type:!=:" + most_common_type, KB)
            elif most_common_resist is not None and (not checked_resist) and (maxValue == most_common_resist_value or checked_weak) :
                checked_resist = True
                ...
                if version == 0:
                    ...
                    answer = ask_question("RESIST_TYPE " + most_common_resist)
                num_asked += 1
                if answer:
                ...
                if version == 0:
                    ...
                    answer = ask_question("WEAK_TYPE " + most_common_weakness)
                num_asked += 1
                if answer:
    ...
    while len(KB) > 1:
        ...
        if version == 0:
            ...
            answer = ask_question("POKEMON " + pokemon_name)
        number_question += 1
        if answer:
